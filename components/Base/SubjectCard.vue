...
<template>
  <div class="w-full bg-background2 rounded-lg mb-3 p-3 overflow-hidden">
    <p class="text-sm text-text inline">
      {{ subject.groups + " " + subject.subgroup }}
    </p>
    ...
    <p class="text-sm text-text inline">
      {{ formatTime(subject.dateTime, subject.type) }}
    </p>
    <p
      v-if="subject.location === 'Онлайн' && subject.url !== ''"
      class="text-right text-sm underline italic cursor-pointer text-blue-600"
    >
      <a :href="subject.url" target="_blank" rel="noopener noreferrer">{{
        subject.location
      }}</a>
    </p>
    <p v-else class="text-sm text-right text-secondary">
      {{ subject.location }}
    </p>
  </div>
</template>
